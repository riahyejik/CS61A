...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # if QueenAnt.true_queen[0] != insect:
            if type(insect) != QueenAnt or QueenAnt.true_queen[0] != insect:
                ...
                insect.place = None        
            if type(insect) == QueenAnt and QueenAnt.true_queen[0] == insect:
                return
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        current = self.place
        distance = 0
        while current is not hive:
            if current.bees and distance in range(self.min_range, self.max_range + 1):
                return random_or_none(current.bees)
            distance += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 4A
        copy_place_bees = self.place.bees[:]
        Ant.reduce_armor(self, amount)
        if self.armor <= 0:
            for bees in copy_place_bees:
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower ants are watersafe and more costly"""
    name = 'Scuba'
    food_cost = 6
    watersafe = True
    implemented = True
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        buffed = 0
        current = self.place
        while current.exit != None and self.true_queen[0] == self:
            current = current.exit
            if current.ant != None:
                if current.ant.container and current.ant.ant != None and current.ant.ant not in self.double_damage: #when container.ant exist
                    current.ant.ant.damage *= 2
                    self.double_damage += [current.ant.ant]
                if current.ant not in self.double_damage:
                    current.ant.damage *= 2
                    self.double_damage += [current.ant]
        if self.true_queen[0] != self:
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 9
        self.armor -= amount
        if self.armor <= 0 and self.true_queen[0] == self:
            bees_win()
        # END Problem 9
